...
Pipeline:
  1. Scrape & analyze SaaS website (Groq)
...
def research_saas_online(url: str, gemini_key: str) -> dict:
    """
    Use Groq with Google Search grounding to deeply research a SaaS product
...
Be thorough. Use REAL data from your search results, not made-up information."""
    ...
    groq_key = os.environ.get("GROQ_API_KEY") or gemini_key

    if not groq_url or not groq_key:
        print("[SaaSShorts] ⚠️ GROQ_API_URL or GROQ_API_KEY not configured; skipping remote research")
    ...
    try:
        ...
        print(f"[SaaSShorts] ⚠️ GROQ research request failed: {e}")
        return {"raw_research": "", "product_name": domain, "grounding_sources": []}

    # Try extracting text output
    raw = data.get("output") or data.get("text") or data.get("response") or resp.text
    if not raw:
        print("[SaaSShorts] ⚠️ GROQ returned empty response for web research")
        return {"raw_research": "", "product_name": domain, "grounding_sources": []}
    ...
    print(f"[SaaSShorts] ✅ Web research complete")
    return research
...
def analyze_saas(scraped_data: dict, gemini_key: str, web_research: dict = None) -> dict:
    ...
    if not groq_url or not groq_key:
        raise Exception("GROQ_API_URL or GROQ_API_KEY not configured for analysis")
...
Include 5-8 pain points, 4-6 emotional hooks, and 4+ viral angles."""
    ...
    resp.raise_for_status()
    try:
        raw = resp.json().get("output") or resp.json().get("text") or resp.text
    except Exception:
        raw = resp.text

    if not raw:
        raise Exception("GROQ returned empty response for SaaS analysis")
...
def generate_scripts(
    ...
    groq_key = os.environ.get("GROQ_API_KEY") or gemini_key
    if not groq_url or not groq_key:
        raise Exception("GROQ_API_URL or GROQ_API_KEY not configured for script generation")
...
- Example male: "a 29 year old european man, short dark hair, light stubble, wearing a navy t-shirt, smart casual look" """
    ...
    if not raw:
        raise Exception("GROQ returned empty response for script generation")
