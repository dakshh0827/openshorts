...
from s3_uploader import upload_job_artifacts, list_all_clips, upload_actor_to_r2, list_actor_gallery, upload_video_to_gallery, list_video_gallery
...
async def process_endpoint(
    ...
    # Prefer a globally-configured Groq API key, allow optional per-request override
    api_key = request.headers.get("X-Groq-Key") or os.environ.get("GROQ_API_KEY")
    if not api_key:
        raise HTTPException(status_code=400, detail="Missing Groq API Key (set GROQ_API_KEY in environment or provide X-Groq-Key header)")
...
@app.post("/api/process_sync")
async def process_sync(
    request: Request,
    file: Optional[UploadFile] = File(None),
    url: Optional[str] = Form(None),
    acknowledged: Optional[str] = Form(None),
    timeout_seconds: Optional[int] = Form(300)
):
    """Accepts a video upload (or URL) and processes it synchronously, returning a ZIP of clips.
    Note: for large videos this may take time; `timeout_seconds` controls max wait.
    """
    # Prefer a globally-configured Groq API key, allow optional per-request override
    api_key = request.headers.get("X-Groq-Key") or os.environ.get("GROQ_API_KEY")
    if not api_key:
    ...
    try:
        ...
        # capture and stream logs to server console
        for line in iter(proc.stdout.readline, b""):
            try:
                decoded = line.decode('utf-8').strip()
                if decoded:
                    print(f"[sync-job] {decoded}")
            except Exception:
                pass
        ...
        if proc.returncode != 0:
            raise HTTPException(status_code=500, detail=f"Processing failed with code {proc.returncode}")

        # Locate metadata and clips
        json_files = glob.glob(os.path.join(job_output_dir, "*_metadata.json"))
        if not json_files:
            # Try relocate rescue
            _relocate_root_job_artifacts(job_id, job_output_dir)
            json_files = glob.glob(os.path.join(job_output_dir, "*_metadata.json"))

        if not json_files:
            raise HTTPException(status_code=500, detail="No metadata produced; cannot collect clips")
        ...
        # Build ZIP on disk to avoid high memory usage (safer for Render)
        ...
        try:
            ...
            return StreamingResponse(iterfile(temp_path), media_type="application/zip", headers=headers)
        except Exception:
            try:
...
async def edit_clip(
    ...
    # Determine API Key (prefer body, then per-request header, then global env)
    final_api_key = req.api_key or x_groq_key or os.environ.get("GROQ_API_KEY")
    
    if not final_api_key:
        raise HTTPException(status_code=400, detail="Missing Groq API Key (provide in body, X-Groq-Key header, or set GROQ_API_KEY in env)")
...
async def generate_effects_config(
    ...
    """Generate structured EffectsConfig JSON for Remotion rendering via Groq LLM."""
    final_api_key = x_groq_key or os.environ.get("GROQ_API_KEY")

    if not final_api_key:
        raise HTTPException(status_code=400, detail="Missing Groq API Key (provide X-Groq-Key header or set GROQ_API_KEY in env)")
    ...
    try:
        ...
        def run_effects_generation():
            ...
            try:
                # Upload video to Groq (via VideoEditor adapter)
                vid_file = editor.upload_video(safe_input_path)
        ...
        if effects_config is None:
            raise HTTPException(status_code=500, detail="Failed to generate effects config from Groq")
...
async def thumbnail_analyze(
    ...
    if not api_key:
        raise HTTPException(status_code=400, detail="Missing Groq API Key (set GROQ_API_KEY or provide X-Groq-Key header)")
...
async def thumbnail_generate(
    ...
    """Generate YouTube thumbnails with Groq image generation (if supported)."""
    api_key = x_groq_key or os.environ.get("GROQ_API_KEY")
    if not api_key:
        raise HTTPException(status_code=400, detail="Missing Groq API Key (set GROQ_API_KEY or provide X-Groq-Key header)")
    ...
    try:
        ...
        if not thumbnails:
            raise HTTPException(status_code=500, detail="Thumbnail generation failed. Please check your Groq API key / endpoint configuration for image generation access.")
    ...
    try:
        ...
        def run_analysis():
            ...
            if req.url and req.url.strip():
                ...
                web_research = research_saas_online(req.url, groq_key)
                analysis = analyze_saas(scraped, groq_key, web_research=web_research)
            else:
            ...
            scripts = generate_scripts(analysis, groq_key, req.num_scripts, req.style, req.language, req.actor_gender)
            return {
                "analysis": analysis,
                "scripts": scripts,
...
async def saasshorts_actor_options(
    ...
    try:
        ...
        # Upload each actor image to public R2 with description
        desc = req.actor_description
        if req.product_description:
            desc += f" (holding {req.product_description})"
        urls = []
        for p in paths:
            r2_url = upload_actor_to_r2(p, description=desc)
            if r2_url:
                urls.append(r2_url)
            else:
                # Fallback to local URL if R2 fails
