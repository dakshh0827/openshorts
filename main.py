...
# Groq integration via HTTP adapter (configured by GROQ_API_URL/GROQ_API_KEY)
...
def get_viral_clips(transcript_result, video_duration):
    ...
    if not api_key:
        ...
        return None
    if not groq_url:
        print("❌ Error: GROQ_API_URL not set. Please configure the Groq HTTP endpoint in GROQ_API_URL.")
        return None
    ...
    if not segments:
        print("❌ Groq Error: No transcript segments found.")
        return None

    chunk_char_budget = int(os.getenv("GROQ_CHUNK_CHAR_BUDGET", "1800"))
    max_words_per_chunk = int(os.getenv("GROQ_CHUNK_WORD_CAP", "90"))
    max_clips_per_chunk = int(os.getenv("GROQ_MAX_CLIPS_PER_CHUNK", "2"))
    max_total_clips = int(os.getenv("GROQ_MAX_TOTAL_CLIPS", "12"))
    max_retry_attempts = int(os.getenv("GROQ_MAX_RETRY_ATTEMPTS", "3"))

    def _extract_text_from_response(resp_json, raw_text):
        ...
        if isinstance(resp_json, dict) and resp_json.get('output'):
    ...
    def _safe_parse_json(text):
        ...
        start_idx = text.find('{')
        end_idx = text.rfind('}')
        body = text[start_idx:end_idx + 1] if start_idx != -1 and end_idx != -1 else text
        try:
            return json.loads(body)
        except Exception:
            return None

    def _build_chunks(all_segments):
        chunks = []
        current = []
        current_chars = 0
        for seg in all_segments:
            ...
            if current and current_chars + seg_len > chunk_char_budget:
    ...
    def _chunk_words(chunk_segments):
        words = []
        step = 1
        total_words = sum(len(seg.get('words', [])) for seg in chunk_segments)
        if total_words > max_words_per_chunk:
            step = max(2, total_words // max_words_per_chunk)
    ...
    def _clip_iou(a_start, a_end, b_start, b_end):
        inter = max(0.0, min(a_end, b_end) - max(a_start, b_start))
        union = max(a_end, b_end) - min(a_start, b_start)
        return (inter / union) if union > 0 else 0.0

    def _sleep_for_rate_limit(resp_text):
        match = re.search(r"try again in ([0-9]+(?:\.[0-9]+)?)s", resp_text or "", re.IGNORECASE)
        if match:
            wait_seconds = float(match.group(1)) + 1.0
        else:
            wait_seconds = 12.0
        print(f"⏳ Groq rate limit hit. Waiting {wait_seconds:.1f}s before retrying...")
        time.sleep(wait_seconds)

    def _call_chunk(prompt_text, chunk_index, total_chunks):
        payload = {
            "model": model_name,
        ...
        for attempt in range(1, max_retry_attempts + 1):
            ...
            if resp.status_code == 429:
                print(f"⚠️  Groq chunk {chunk_index}/{total_chunks} rate-limited on attempt {attempt}/{max_retry_attempts}: {resp.text}")
                if attempt < max_retry_attempts:
                    _sleep_for_rate_limit(resp.text)
                    continue

            print(f"❌ Groq chunk {chunk_index}/{total_chunks} error ({resp.status_code}): {resp.text}")
            return None

        return None

    chunks = _build_chunks(segments)
    print(f"🤖  Transcript chunking enabled: {len(chunks)} chunk(s)")

    candidates = []
    for idx, chunk in enumerate(chunks, start=1):
        chunk_start = float(chunk[0].get('start', 0.0))
        chunk_end = float(chunk[-1].get('end', video_duration))
        chunk_text = " ".join((s.get('text') or '').strip() for s in chunk).strip()
        chunk_words = _chunk_words(chunk)
        chunk_prompt = f"""
Select up to {max_clips_per_chunk} viral short-form clip candidates from this transcript chunk.

Rules:
- Return ABSOLUTE seconds from full video timeline.
- Each clip must be 15-60 seconds.
- Ensure {chunk_start:.3f} <= start < end <= {chunk_end:.3f}.
- Return only JSON.
- Keep output compact and focused.

VIDEO_DURATION_SECONDS: {video_duration}
CHUNK_INDEX: {idx}/{len(chunks)}
CHUNK_TIME_RANGE_SECONDS: {chunk_start:.3f}-{chunk_end:.3f}

TRANSCRIPT_TEXT:
{json.dumps(chunk_text)}
...
""".strip()
        ...
        try:
            resp = _call_chunk(chunk_prompt, idx, len(chunks))
            if not resp:
                continue

            data = resp.json()
            text = _extract_text_from_response(data, resp.text)
            parsed = _safe_parse_json(text)
            if not parsed or 'shorts' not in parsed:
                print(f"⚠️  Groq chunk {idx}/{len(chunks)} returned unparsable response.")
                continue

            for clip in parsed.get('shorts', []):
                try:
                ...
                start = max(0.0, min(start, float(video_duration)))
                end = max(0.0, min(end, float(video_duration)))
                duration = end - start
                if duration < 15 or duration > 60 or start >= end:
                    continue

                try:
                    score = int(clip.get('viral_score', 50))
                except Exception:
                    score = 50

                clip['start'] = round(start, 3)
                clip['end'] = round(end, 3)
                clip['viral_score'] = max(0, min(score, 100))
                candidates.append(clip)

            print(f"✅ Groq chunk {idx}/{len(chunks)} processed.")
        except Exception as e:
            print(f"❌ Groq chunk {idx}/{len(chunks)} exception: {e}")

    if not candidates:
        print("❌ Groq Error: No clip candidates produced from chunks.")
        return None

    # Rank by score and de-duplicate overlapping clips.
    candidates.sort(key=lambda c: c.get('viral_score', 0), reverse=True)
    deduped = []
    for clip in candidates:
        overlap = False
        for kept in deduped:
            if _clip_iou(clip['start'], clip['end'], kept['start'], kept['end']) >= 0.6:
                overlap = True
                break
        if not overlap:
            deduped.append(clip)
    ...
    for clip in deduped:
        clip.pop('viral_score', None)

    if not deduped:
        print("❌ Groq Error: All candidate clips were filtered out.")
        return None

    return {"shorts": deduped}
    ...
    if args.skip_analysis:
        ...
        if not clips_data or 'shorts' not in clips_data:
            ...
            for i, clip in enumerate(clips_data['shorts']):
                ...
                if success:
                    # Burn subtitles onto the final clip using the source transcript
                    srt_path = os.path.join(output_dir, f"{video_title}_clip_{i+1}.srt")
                    subtitled_temp_path = os.path.join(output_dir, f"temp_subtitled_{clip_filename}")
                    try:
                        subtitle_ready = generate_srt(transcript, start, end, srt_path)
                        if subtitle_ready:
                            burn_subtitles(
                                clip_final_path,
                                srt_path,
                            ...
                            os.replace(subtitled_temp_path, clip_final_path)
                            print(f"   💬 Subtitles baked into: {clip_final_path}")
                        else:
                            print(f"   ⚠️ No subtitle words found for clip {i+1}; keeping clip without subtitles.")
                    except Exception as subtitle_error:
                        print(f"   ⚠️ Subtitle burn failed for clip {i+1}: {subtitle_error}")
                    finally:
                        if os.path.exists(srt_path):
                            try:
                                ...
                                pass
                        if os.path.exists(subtitled_temp_path):
                            try:
                                os.remove(subtitled_temp_path)
                            except Exception:
                                pass
