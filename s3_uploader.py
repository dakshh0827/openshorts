...
def upload_file_to_r2(file_path, bucket_name, r2_key):
    """
    Upload a file to a Cloudflare R2 bucket silently.
    """
...
def get_r2_client():
    """Returns an authenticated Cloudflare R2 client."""
    access_key = os.environ.get('R2_ACCESS_KEY_ID')
    secret_key = os.environ.get('R2_SECRET_ACCESS_KEY')
    account_id = os.environ.get('R2_ACCOUNT_ID')
...
def generate_presigned_url(bucket_name, object_key, expiration=3600):
    """Generate a presigned URL to share a Cloudflare R2 object."""
    r2_client = get_r2_client()
    if not r2_client:
        return None
    try:
        response = r2_client.generate_presigned_url('get_object',
...
def list_all_clips(bucket_name=None, limit=50, force_refresh=False):
    """
    List recent clips from the Cloudflare R2 bucket by finding metadata files.
    Returns a list of dicts containing clip info and signed URLs.
    
    Args:
        bucket_name: R2 bucket name (defaults to R2_BUCKET env var)
    ...
    if not bucket_name:
        bucket_name = os.environ.get('R2_BUCKET', 'my-clips-bucket')
    ...
    try:
        ...
        for meta_obj in metadata_files:
            ...
            try:
                obj_resp = r2_client.get_object(Bucket=bucket_name, Key=key)
...
def upload_actor_to_r2(file_path, description=""):
    """
    Upload an actor image to the public Cloudflare R2 bucket.
    Returns the public URL or None on failure.
    """
    bucket_name = os.environ.get('R2_PUBLIC_BUCKET', 'my-public-bucket')
    domain = os.environ.get('R2_DOMAIN')
    ...
    try:
        ...
        # Use custom domain if provided, otherwise use R2 default
        if domain:
            public_url = f"https://{domain}/{r2_key}"
        else:
            public_url = f"https://{bucket_name}.r2.cloudflarestorage.com/{r2_key}"
        ...
        logger.info(f"Uploaded actor to R2: {public_url}")
        return public_url
    except Exception as e:
        logger.error(f"Failed to upload actor to R2: {e}")
        return None


def list_actor_gallery():
    """
    List all actor images from the public Cloudflare R2 bucket.
    ...
    try:
        ...
        for base, data in all_objects.items():
            ...
            if domain:
                ...
                public_url = f"https://{bucket_name}.r2.cloudflarestorage.com/{key}"
            entry = {
                "url": public_url,
                "key": key,
            ...
            if 'meta_key' in data:
                try:
                    meta_resp = r2_client.get_object(Bucket=bucket_name, Key=data['meta_key'])
...
def upload_video_to_gallery(video_path, actor_image_path, metadata, video_id=None):
    """
    Upload a generated UGC video + actor + metadata to the public Cloudflare R2 bucket.
    ...
    if domain:
        ...
        base_url = f"https://{bucket_name}.r2.cloudflarestorage.com"
    results = {}

    try:
        # Upload video
        if os.path.exists(video_path):
            r2_key = f"videos/{video_id}/video.mp4"
            r2_client.upload_file(video_path, bucket_name, r2_key,
                                 ExtraArgs={'ContentType': 'video/mp4'})
            results["video_url"] = f"{base_url}/{r2_key}"

        # Upload actor image
        if actor_image_path and os.path.exists(actor_image_path):
            r2_key = f"videos/{video_id}/actor.png"
            r2_client.upload_file(actor_image_path, bucket_name, r2_key,
                                 ExtraArgs={'ContentType': 'image/png'})
            results["actor_url"] = f"{base_url}/{r2_key}"
        ...
        r2_key = f"videos/{video_id}/metadata.json"
        r2_client.put_object(
            Bucket=bucket_name, Key=r2_key,
            Body=meta_json.encode('utf-8'),
            ContentType='application/json',
        )
        results["metadata_url"] = f"{base_url}/{r2_key}"
...
def list_video_gallery(limit=50, force_refresh=False):
    """
    List all UGC videos from the public Cloudflare R2 bucket.
    ...
    try:
        ...
        for meta_obj in meta_files:
            try:
                obj_resp = r2_client.get_object(Bucket=bucket_name, Key=meta_obj['Key'])
                content = obj_resp['Body'].read().decode('utf-8')
                data = json.loads(content)
                videos.append(data)
...
def upload_job_artifacts(directory, job_id):
    """
    Upload all generated clips and metadata for a job to Cloudflare R2.
    """
    bucket_name = os.environ.get('R2_BUCKET', 'my-clips-bucket')
    ...
    for filename in os.listdir(directory):
        ...
        if (filename.endswith(".mp4") or filename.endswith(".json")) and not filename.startswith("temp_"):
            ...
            upload_file_to_r2(file_path, bucket_name, r2_key)
