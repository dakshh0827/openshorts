...
# Simple Groq HTTP adapter: expects `GROQ_API_URL` and `GROQ_API_KEY` in env
GROQ_API_URL = os.environ.get("GROQ_API_URL", "")
GROQ_API_KEY = os.environ.get("GROQ_API_KEY", "")

class VideoEditor:
    def __init__(self, api_key=None):
        # If api_key not provided, fall back to global GROQ_API_KEY
        self.api_key = api_key or GROQ_API_KEY
        self.model_name = os.environ.get("GROQ_MODEL", "groq-1")

    def upload_video(self, video_path):
        """Uploads video - with Groq adapter we keep local path and don't perform remote file upload."""
        print(f"📤 Preparing {video_path} for Groq (local path used in prompt)...")
        ...
        # For Groq integration we do not perform file upload in this adapter.
        # Return the local path; downstream methods will reference the path in prompts.
        return video_path

    def get_ffmpeg_filter(self, video_file_obj, duration, fps=30, width=None, height=None, transcript=None):
        """Asks Groq for a raw FFmpeg filter string."""
        ...
        print("🤖 Asking Groq for FFmpeg filter...")
    ...
    def get_effects_config(self, video_file_obj, duration, fps=30, width=None, height=None, transcript=None):
        """Asks Groq for a structured EffectsConfig JSON for Remotion rendering."""
        if width is None or height is None:
            width, height = 1080, 1920
        ...
        print("🤖 Asking Groq for Remotion effects config...")
        response_text = self._call_groq(prompt)
        print(f"🔍 DEBUG: Groq Raw Response:\n{response_text}")
    ...
    def _call_groq(self, prompt: str) -> str:
        ...
        Set `GROQ_API_URL` and `GROQ_API_KEY` in your environment.
        """
        if not GROQ_API_URL or not self.api_key:
            raise Exception("GROQ_API_URL and GROQ_API_KEY must be set in environment to call Groq.")
    ...
    def _sanitize_filter_string(filter_string: str) -> str:
        """
        Best-effort sanitizer for Groq-generated FFmpeg expressions.
